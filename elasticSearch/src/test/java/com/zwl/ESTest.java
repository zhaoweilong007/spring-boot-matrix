...
public class ESTest {
  ...
  public void document() {
    ...
    BlogModel model =
        new BlogModel().setId("1").setTitle("前后就饿看航空机枪和接口").setContent("大苏打撒旦").setUrl("请问请问");
  ...
  public void search() {
    ...
    SearchHits<BlogModel> criteriaSearchHits =
        elasticsearchRestTemplate.search(criteriaQuery, BlogModel.class);
    Console.log("criteria query hits :{}", criteriaSearchHits);

    Query stringQuery = new StringQuery("{ \"match\": { \"title\": { \"query\": \"ja\" } } } ");
    SearchHits<BlogModel> stringQuerySearchHits =
        elasticsearchOperations.search(stringQuery, BlogModel.class);
    Console.log("stringQuerySearchHits:{}", stringQuerySearchHits);

    NativeSearchQuery nativeSearchQuery =
        new NativeSearchQueryBuilder().withQuery(QueryBuilders.termQuery("title", "java")).build();

    SearchHits<BlogModel> blogModelSearchHits =
        elasticsearchRestTemplate.search(nativeSearchQuery, BlogModel.class);
