...
public class ListGroupReqHandler extends SimpleChannelInboundHandler<ListGroupReqPacket> {
  ...
  protected void channelRead0(
      ChannelHandlerContext channelHandlerContext, ListGroupReqPacket listGroupReqPacket)
      throws Exception {
    ...
    if (groupMap == null) {
      channelHandlerContext.writeAndFlush(packetBuilder.success(false).msg("群聊不存在").build());
    } else {
      List<String> list =
          groupMap.stream()
              .map(
